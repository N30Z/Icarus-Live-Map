...
def _is_heap_addr(addr: int, mod: ModuleInfo) -> bool:
    """Loose heap check — validates address range only, no vtable.
    Wine pawn/component vtables often live outside the module range,
    so _is_heap_uobj rejects valid pawns. Use this for pawn/comp."""
    return (0x10000 <= addr <= _ADDR_MAX and
            not (mod.base <= addr < mod.base + mod.size))
...
def _scan_player_name(ps_ptr: int) -> str:
    ...
    if name and name.isascii() and name.isprintable() and len(name) > 1:
        return name

    # Slow path: scan object for a plausible FString (data_ptr + len + max).
    # Require ASCII so we don't accept garbage UTF-16 that decodes to Korean/CJK chars.
    # Also verify the null terminator at the expected position.
    raw = read_bytes(ps_ptr, 0x800)
    for off in range(0x80, len(raw) - 12, 8):
        data_ptr, = struct.unpack_from("<Q", raw, off)
        if not (0x10000 <= data_ptr <= _ADDR_MAX):
            continue
        ...
        if length <= 1 or length > 64:
            continue
        # Verify null terminator (UE4 FString length includes the null)
        nt = read_bytes(data_ptr + (length - 1) * 2, 2)
        if nt != b'\x00\x00':
            continue
        str_raw = read_bytes(data_ptr, (length - 1) * 2)
        try:
            s = str_raw.decode("utf-16-le", errors="strict").rstrip("\x00")
        except Exception:
            continue
        if s and s.isascii() and s.isprintable() and len(s) > 1 and not s.startswith("/"):
...
def read_players(gworld_ptr_addr: int, mod: ModuleInfo, offsets: dict) -> list:
    ...
    players = []
    seen_pawns = set()   # detect duplicate pawn pointers (wrong OFF_PAWN_PRIVATE)
    seen_comps = set()   # detect duplicate component pointers (wrong OFF_ROOT_COMPONENT)
    bad_offsets = False
    for i in range(arr_count):
        ...
        # Use loose heap check: Wine pawn vtables live outside the module,
        # so _is_heap_uobj (which checks the vtable) falsely rejects valid pawns.
        if not _is_heap_addr(pawn, mod):
            players.append({"name": name, "online": False})
            continue
        # Duplicate pawn address → multiple PlayerStates share the same pawn.
        # This means OFF_PAWN_PRIVATE is wrong for this build — run --trace.
        if pawn in seen_pawns:
            print(f"  [{name}] Duplicate pawn @ 0x{pawn:X} — OFF_PAWN_PRIVATE wrong, run --trace",
                  file=sys.stderr)
            bad_offsets = True
            players.append({"name": name, "online": False})
            continue
        ...
        if not _is_heap_addr(comp, mod):
            players.append({"name": name, "online": False})
            continue
        # Duplicate component address → same problem, different offset level.
        if comp in seen_comps:
            print(f"  [{name}] Duplicate component @ 0x{comp:X} — OFF_ROOT_COMPONENT wrong, run --trace",
                  file=sys.stderr)
            bad_offsets = True
            players.append({"name": name, "online": False})
            continue
        ...
        # Sanity check: coordinates outside ±6000 m indicate a garbage pointer
        if not (abs(x) < 600_000 and abs(y) < 600_000):
            players.append({"name": name, "online": False})
            continue
    ...
    if bad_offsets:
        print("[!] Duplicate pawn/component pointers detected — offsets are wrong for this build.",
              file=sys.stderr)
        print("[!] Go to /setup → Section 4 → 'Run --trace' to discover correct offsets.",
              file=sys.stderr)
    return players
...
def trace_offsets(gworld_ptr_addr: int, mod: ModuleInfo, ref_json: str,
                  player_filter: str = "") -> dict | None:
    """
    ...
    if player_filter:
        filtered = [t for t in targets if t[0].lower() == player_filter.lower()]
        if filtered:
            print(f"[i] Tracing player: {filtered[0][0]}", file=sys.stderr)
            targets = filtered
        else:
            names = ", ".join(t[0] for t in targets)
            print(f"[!] Player '{player_filter}' not in ref (online: {names}) — using all",
                  file=sys.stderr)
    ...
    # Bottom-up approach: walk PS → Pawn → Component and verify FVector.
    # Does NOT scan all of RAM — avoids the 5+ GB global scan entirely.
    # Does NOT require vtable detection (broken for Wine builds).
    # Validates by checking if candidate component contains known coordinates.
    TOL = 500.0   # 5 m — tight enough to avoid false positives

    # Scan size limits
    PS_SCAN   = 0x4000   # 16 KB read buffer (pawn ptr searched in subset below)
    PAWN_SCAN = 0x6000   # 24 KB  (OFF_ROOT_COMPONENT default 0x198)
    COMP_SCAN = 0x600    # 1.5 KB (OFF_REL_LOCATION default 0x11C)

    # Minimum offsets based on UE4 object layout:
    # - The first ~0x28 bytes of any UObject are header fields (vtable, flags, class ptr…).
    #   APawn* cannot live there, so start pawn search at 0x100 to skip header + AActor base.
    # - USceneComponent* in APawn is similarly deep; start comp search at 0x80.
    # - RelativeLocation in USceneComponent is typically ≥0x100; start loc search at 0xA0.
    #   (Standard build: 0x11C.  Wine/Proton may differ but 0x14 is definitely wrong.)
    PAWN_OFF_MIN = 0x100
    PAWN_OFF_MAX = 0x1000   # pawn ptr is never 4 KB+ into a PlayerState
    COMP_OFF_MIN = 0x80     # component ptr is never in the first 128 bytes of APawn
    LOC_OFF_MIN  = 0xA0     # RelativeLocation is never in the first 160 bytes of USceneComponent
    ...
    for pi in range(arr_count):
        ...
        print(f"[~] Reference: {name}  X={xf/100:.2f}m  Y={yf/100:.2f}m  Z={zf/100:.2f}m",
              file=sys.stderr)
        print(f"[~] Bottom-up scan: PS[{pi}] "
              f"→ pawn (0x{PAWN_OFF_MIN:X}–0x{PAWN_OFF_MAX:X}) "
              f"→ comp (0x{COMP_OFF_MIN:X}+) "
              f"→ loc (0x{LOC_OFF_MIN:X}+) ...", file=sys.stderr)

        ps_raw = read_bytes(ps_ptr, PS_SCAN)
        checked_comps = set()    # comp addrs confirmed not to contain FVector
        comp_cache    = {}       # comp addr → raw bytes (avoid re-reading)
        n_pawn = 0

        for pawn_off in range(PAWN_OFF_MIN, PAWN_OFF_MAX - 8, 8):
            pawn_addr, = struct.unpack_from("<Q", ps_raw, pawn_off)
            # Loose heap check — no vtable requirement; Wine pawn vtables
            # often don't reside in the module and break _is_heap_uobj.
            if not (0x10000 <= pawn_addr <= _ADDR_MAX):
                continue
            if mod.base <= pawn_addr < mod.base + mod.size:
                continue

            # Cross-check: if every other PlayerState in the array has the SAME
            # value at pawn_off, this is a shared field (vtable slot, class ptr, etc.)
            # and cannot be the per-player APawn pointer.
            if arr_count > 1:
                other_ps_ptrs = [read_ptr(arr_data + vj * 8)
                                 for vj in range(arr_count) if vj != pi]
                shared = all(
                    ops and read_ptr(ops + pawn_off) == pawn_addr
                    for ops in other_ps_ptrs if ops
                )
                if shared:
                    continue   # same value in all PlayerStates → not the pawn ptr

            n_pawn += 1
            if n_pawn % 50 == 0:
                print(f"  [~] {n_pawn} pawn candidates ...", file=sys.stderr)

            pawn_raw = read_bytes(pawn_addr, PAWN_SCAN)
            found_false_positive = False
            for comp_off in range(COMP_OFF_MIN, PAWN_SCAN - 8, 8):
                if found_false_positive:
                    break
                comp_addr, = struct.unpack_from("<Q", pawn_raw, comp_off)
                if not (0x10000 <= comp_addr <= _ADDR_MAX):
                    continue
                if mod.base <= comp_addr < mod.base + mod.size:
                    continue
                if comp_addr in checked_comps:
                    continue   # already verified this doesn't contain our FVector

                # Read and cache component bytes
                if comp_addr not in comp_cache:
                    comp_cache[comp_addr] = read_bytes(comp_addr, COMP_SCAN)
                comp_raw = comp_cache[comp_addr]

                # Search at every 4-byte-aligned offset for the FVector triplet
                for loc_off in range(LOC_OFF_MIN, COMP_SCAN - 12, 4):
                    cx, cy, cz = struct.unpack_from("<fff", comp_raw, loc_off)
                    if not (abs(cx - xf) < TOL and abs(cy - yf) < TOL
                            and abs(cz - zf) < TOL):
                        continue
                    # Sanity: coordinates must be within map bounds
                    if abs(cx) > MAP_LIMIT_CM or abs(cy) > MAP_LIMIT_CM:
                        continue

                    # Cross-validate: apply (pawn_off, comp_off, loc_off) to ALL
                    # PlayerStates and check for duplicate output coordinates.
                    # Duplicate coords mean the offsets hit a shared object — false positive.
                    if arr_count > 1:
                        seen_cv = set()
                        is_dup  = False
                        for vj in range(arr_count):
                            vps = read_ptr(arr_data + vj * 8)
                            if not vps:
                                continue
                            vpawn = read_ptr(vps + pawn_off)
                            if not (0x10000 <= vpawn <= _ADDR_MAX):
                                continue
                            vcomp = read_ptr(vpawn + comp_off)
                            if not (0x10000 <= vcomp <= _ADDR_MAX):
                                continue
                            vraw = read_bytes(vcomp + loc_off, 12)
                            vcx, vcy, vcz = struct.unpack("<fff", vraw)
                            if abs(vcx) < MAP_LIMIT_CM and abs(vcy) < MAP_LIMIT_CM:
                                key = (round(vcx, 0), round(vcy, 0))
                                if key in seen_cv:
                                    is_dup = True
                                    break
                                seen_cv.add(key)
                        if is_dup:
                            print(f"  [!] Cross-val: pawn=0x{pawn_off:X} comp=0x{comp_off:X} "
                                  f"loc=0x{loc_off:X} → duplicate coords → false positive",
                                  file=sys.stderr)
                            found_false_positive = True
                            break  # skip remaining loc_offs for this comp

                    print(f"\n  *** OFFSET CHAIN FOUND ***", file=sys.stderr)
                    print(f"  PlayerState + 0x{pawn_off:03X}  → Pawn @ 0x{pawn_addr:X}",
                          file=sys.stderr)
                    print(f"  Pawn        + 0x{comp_off:03X}  → Component @ 0x{comp_addr:X}",
                          file=sys.stderr)
                    print(f"  Component   + 0x{loc_off:03X}  → "
                          f"FVector ({cx/100:.2f}m, {cy/100:.2f}m, {cz/100:.2f}m)",
                          file=sys.stderr)
                    print(f"\n  OFF_PAWN_PRIVATE   = 0x{pawn_off:03X}", file=sys.stderr)
                    print(f"  OFF_ROOT_COMPONENT = 0x{comp_off:03X}", file=sys.stderr)
                    print(f"  OFF_REL_LOCATION   = 0x{loc_off:03X}", file=sys.stderr)
                    offsets = {
                ...
                if not found_false_positive:
                    checked_comps.add(comp_addr)

        print(f"[!] No chain found for player {pi} "
              f"({n_pawn} pawn candidates, {len(checked_comps)} unique comps checked)",
              file=sys.stderr)
...
def main():
    ...
    ap.add_argument("--player", default="",
                    help="Limit --trace to this player name (exact match, case-insensitive)")
    ap.add_argument("--loop", type=float, default=0,
                    help="Loop interval in seconds (0 = run once)")
    ap.add_argument("--pawn", default="",
                    help="Override OFF_PAWN_PRIVATE offset (hex, e.g. 0x20)")
    ap.add_argument("--comp", default="",
                    help="Override OFF_ROOT_COMPONENT offset (hex, e.g. 0x27c0)")
    ap.add_argument("--loc",  default="",
                    help="Override OFF_REL_LOCATION offset (hex, e.g. 0x450)")
    args = ap.parse_args()
    ...
    if args.trace:
        result = trace_offsets(gworld_ptr, mod, args.ref, args.player)
        if result:
    ...
    if args.pawn: offsets["OFF_PAWN_PRIVATE"]   = int(args.pawn, 16)
    if args.comp: offsets["OFF_ROOT_COMPONENT"] = int(args.comp, 16)
    if args.loc:  offsets["OFF_REL_LOCATION"]   = int(args.loc,  16)
