...
def _auto_populate_ref(ref_path: str):
    """Populate ref_path from the live savegame if it has no online players."""
    # Check if existing file is already usable
    try:
        if os.path.exists(ref_path):
            with open(ref_path, encoding="utf-8") as f:
                data = json.load(f)
            pl_list = data.get("players") or (data if isinstance(data, list) else [])
            if any(pl.get("online") for pl in pl_list):
                return  # already good
    except Exception:
        pass  # fall through and try savegame

    # Try to read from the live GD.json
    try:
        gd     = _read_live_gd_json()
        binary = p.gd_json_to_binary(gd)
        players = p.extract_players_compat(binary)
        ...
        print(f"[trace] Auto-populated {ref_path} with {len(online)} online player(s)")
    except Exception as e:
...
class Handler(http.server.BaseHTTPRequestHandler):
            ...
            try:
                ...
                if mode == "host_memory":
                    ...
                    with open(ref_path, "wb") as f:
                        f.write(ref_data)
                    self._json({"ok": True, "online": len(online), "path": ref_path})
                else:
    ...
    def _serve_static(self, url_path):
        ...
        full = os.path.normpath(os.path.join(PROJECT_ROOT, safe))
        if not full.startswith(os.path.abspath(PROJECT_ROOT)):
